...
import furhatos.app.fortuneteller.flow.main.DeepSleep
import furhatos.app.fortuneteller.flow.main.Idle
import furhatos.app.fortuneteller.flow.main.Nap
import furhatos.app.fortuneteller.flow.noInput
import furhatos.app.fortuneteller.flow.noMatch
import furhatos.app.fortuneteller.utils.NoUsersPresentEvent
import furhatos.app.fortuneteller.utils.RandomHeadEvent
        ...
        if (users.hasCurrent()) { // There is a user that furhat is attending
            when {
                ...
                users.usersAttendingFurhat.isNotEmpty() -> { // no one is attending furhat we are expected to return to wait for user attention
                    goto(Start) // go to intro to wait for user attention
                }
